...
2.  "Income" -- median household income in 2010; the data come from the Census, gathered through NHGIS

3.  "Insurance" -- population counts of insured and uninsured people by county, broken down by age and sex, in 2010; this data also comes from the Census, gathered through NHGIS
...
We want to find the uninsured rate for people under the age of 65, since people over the age of 65 are usually covered by Medicare. Go to the "Insurance" sheet. **Make an empty column** between the COUNTYA column and Total column. To do this, click on the letter of the column to the right of where you want to make a column. (See Figure 1.)
...
Explore the uninsured rate a little bit. In an empty cell in the "Insurance" sheet, type `=AVERAGE(`. Then, select all of the values for the uninsured rate. Select cell G4, then hit **"Ctrl+Shift+Down Arrow" (or "Command+Shift+Down Arrow" for Mac)**. Then, hit enter. You should get the answer 16.7%. Also calculate the median using `=MEDIAN()`.
