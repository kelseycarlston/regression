...
uninsured <- insurance |> 
  ...
  mutate(fips = STATEA*1000 + COUNTYA) |> 
  clean_names() |> 
  group_by(fips) |> 
...
inc <- inc |> 
  ...
  clean_names() |> 
  select(state, fips, median_hh_income) |> 
  left_join(le |> 
              filter(Year == 2010) |> 
              clean_names() |> 
              select(fips, female_life_expectancy_years),
            by = "fips") |> 
  rename(female_le = female_life_expectancy_years) |> 
  left_join(uninsured, by = "fips") |> 
